...
FEATURE_DIM = 103  # v66: v60 19 dim + 84 dim full-logit stacker (codex idea #1)
# Layout:
...
#   dim 19..39: 11-student ensemble mean prob[21] (full-logit stacker)
#   dim 40..60: 11-student ensemble std prob[21]
#   dim 61..81: 11-student vote_hist[21] (count_c / 11)
#   dim 82..102: v63 prob[21] (single strongest student)
...
class _AccentDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        reweighted = 0
        total_tokens = 0
        for utt, tpred, tlog, mfeat, cons in zip(
            utterances, tpreds, tlogits, metafs, consensus_list, strict=True
        ):
            ...
            for i, m in enumerate(morphemes):
                ...
                # v66: meta_features now contains the 84-dim full-logit stacker.
                # FEATURE_DIM (103) - 13 base - 1 argmax - 5 soft = 84 expected.
                stacker_dim = FEATURE_DIM - 13 - 1 - 5
                if mfeat is not None and i < len(mfeat):
                    meta_extra = [float(x) for x in mfeat[i].tolist()]
                    if len(meta_extra) != stacker_dim:
                        # Pad/truncate to expected dim
                        if len(meta_extra) < stacker_dim:
                            meta_extra = meta_extra + [0.0] * (
                                stacker_dim - len(meta_extra)
                            )
                        else:
                            meta_extra = meta_extra[:stacker_dim]
                else:
                    meta_extra = [0.0] * stacker_dim
...
def main() -> None:
    ...
    parser.add_argument(
        "--meta-cache",
        type=str,
        default="/tmp/v66_stacker.pt",
        help="v66: precomputed 84-dim full-logit stacker features path",
    )
    parser.add_argument(
        "--ord-alpha",
        type=float,
    ...
    print("=" * 65)
    print(
        "kotonoha accent model trainer v66 "
        "(v62 + token-level selective reweight from 9-student consensus)"
    )
