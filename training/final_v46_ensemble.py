"""Final ensemble with v46 included."""
...
def main() -> None:
    ...
    # Also add TTA variants for top 2
    tta_models = ["v20", "v17", "v46"]
    ...
    for utt in val_utts:
        ...
        for tm in tta_models:
            ...
            for _ in range(n_aug):
                aug = base_inp.copy()
                noise = rng.normal(0, 0.02, size=aug[:, 5:].shape).astype(np.float32)
                aug[:, 5:] += noise
    ...
    print(f"\nTotal morphemes: {len(flat_labels)}")
    ...
    # Random weighted
    print(f"\nRandom weighted search on {len(best_members)} members (5000 trials)...")
    best_wacc = best_acc
