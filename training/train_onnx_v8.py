"""kotonoha ONNX accent model trainer (v8).

v7ベースに以下の改善を加えた学習パイプライン:
- SAM (Sharpness-Aware Minimization) — 平坦な最小値を探索し汎化性能向上
- Multi-seed Best-of-N（複数seedで独立学習→最良を選択）
- ReduceLROnPlateau（デフォルト）/ CosineAnnealingLR
- Top-3 checkpoint averaging（v7と同じ）
- R-Drop正則化（v7から継承）
- LLMコーパスデータのStage 2混合学習（v7から継承）

Stage 1: JVS データで事前学習
Stage 2: JSUT + コーパスデータで微調整（Top-K + Cosine LR）
...
Usage:
  ...
  cd training
  uv run python train_onnx_v8.py                    # default: 8 seeds
  uv run python train_onnx_v8.py --seeds 0,1,2      # 3 seeds
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v8")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v8")
    print("  Arch: BiLSTM(3) + SelfAttn(4h) + Emb + 2-MLP")
