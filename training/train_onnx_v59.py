r"""kotonoha ONNX accent model trainer (v59 — disjoint K-fold CV).

v38 setting (14 dim feature with v24 teacher argmax) を base に、
disjoint K-fold CV を導入。真の no-leak generalization を測定するため、
JSUT データを K (=5) 個の重複なし fold に分割し、各 fold で
`--fold-id N` 指定で 1 model を学習する。全 fold の OOF prediction を
集計すれば、データ全体での真の精度が得られる。

## 1. Disjoint K-fold split
- 固定 seed (`--fold-base-seed 0`) で indices を一度 shuffle
- 5 fold で重複なく分割。各 fold ~10% (val_size = len/5)
- `--fold-id 0..4` で対応 fold を val、残りを train として使う
- 5 model 学習時に val が disjoint (val_split_seed=N とは異なる)

## 2. v38 から継承 (14 dim teacher feature)
- v24 teacher argmax を 14 次元目の feature として使う
- 6-seed best-of-N、120 ep cosine、Top-10、greedy soup、EMA、SWA

## 3. OOF eval は別 script で実施
- 各 fold の最良 state を集めて、対応 val set で eval、集計

特徴量 14 dim layout:
  [pos_id, pd1_id, pd2_id, ct_id, cf_id,
   mora_count, reading_hash, first_char_hash,
   last_char_hash, position, dict_accent_type,
   reading_head2_hash, reading_tail2_hash, teacher_pred/20.0]

Usage:
  cd training
  for FOLD in 0 1 2 3 4; do
    uv run python train_onnx_v59.py --fold-id $FOLD --seeds 0,1,2 \
      --state-dir /tmp/v59_states \
      --output /mnt/c/GitHub/kotonoha-models/accent_model_v59_f$FOLD.onnx
  done
"""
...
def _kfold_val_idx(
    n: int, fold_id: int, num_folds: int = 5, fold_base_seed: int = 0
) -> set[int]:
    """Disjoint K-fold split: 固定 seed で shuffle し fold_id の slice を返す.

    Args:
        n: データ数.
        fold_id: 0..num_folds-1.
        num_folds: 分割数.
        fold_base_seed: shuffle 用 seed.

    Returns:
        当該 fold の val 用 index 集合.

    """
    rng = random.Random(fold_base_seed)
    indices = list(range(n))
    rng.shuffle(indices)
    fold_size = n // num_folds
    start = fold_id * fold_size
    end = start + fold_size if fold_id < num_folds - 1 else n
    return set(indices[start:end])
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    # ── Val split (v59: disjoint K-fold or v31 legacy split) ──
    if fold_id is not None:
        val_idx = _kfold_val_idx(
            len(jsut_utterances), fold_id, num_folds, fold_base_seed
        )
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="v59: disjoint K-fold id (0..num_folds-1). Overrides --val-split-seed.",
    )
    parser.add_argument(
        "--num-folds",
        type=int,
        default=5,
        help="v59: number of folds for disjoint K-fold CV.",
    )
    parser.add_argument(
        "--fold-base-seed",
        type=int,
        default=0,
        help="v59: shuffle seed used to permute indices before fold assignment.",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v38 (14-dim + teacher argmax feature)")
    ...
    for i, seed in enumerate(seeds):
        ...
        if args.resume and state_path.exists():
            ...
            acc, state = _train_single(
                ...
                fold_base_seed=args.fold_base_seed,
            )
