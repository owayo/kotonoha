"""kotonoha ONNX accent model trainer (v12).

v11ベースに過学習ギャップ解消を目的とした改善:

過学習対策 (train 82% vs val 71% のギャップ解消):
- モデル容量削減: hidden=192, 2層BiLSTM, embed=48 (~2.5M params, -47%)
- Manifold Mixup: LSTM出力レベルで2サンプルを補間 (alpha=0.2)
- Feature noise: 連続特徴量にガウスノイズ (std=0.03)
- 強化正則化: dropout=0.5, weight_decay=0.1, morpheme_dropout=0.2
- Reading dropout強化: 0.1 -> 0.3
- R-Drop強化: alpha=0.5 -> 1.0
- 5-fold交差検証: 信頼性の高い評価 + 全データ活用

v11から継承:
- SAM, R-Drop, Top-K checkpoint averaging, multi-seed Best-of-N
- ReduceLROnPlateau / Cosine scheduler
...
Usage:
  ...
  cd training
  uv run python train_onnx_v12.py                    # default: 16 seeds, 5-fold CV
  uv run python train_onnx_v12.py --seeds 0,1,2      # 3 seeds
  uv run python train_onnx_v12.py --no-cv             # skip CV, use fixed 90/10 split
  uv run python train_onnx_v12.py --hidden-dim 256    # override capacity (ablation)
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v12: Add Gaussian noise to continuous features (indices 5-10)
...
# ── Model (v12 architecture: compact) ─────────────────────────────────
...
class AccentModel(nn.Module):
    ...
    v12: Compact model for overfitting reduction.
    - hidden=192 (was 256), 2-layer BiLSTM (was 3), embed=48 (was 64)
    - Manifold Mixup at LSTM output level
    - Stronger reading_dropout (0.3 default)
    """
    ...
    def forward(
        ...
        *,
        ...
        Returns:
            logits: [batch, seq_len, num_classes], or (logits, hidden) if return_hidden.

        """
        # v12: Reading dropout -- force model to learn without kana_emb
        if (
            self.training
        ...
        # Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h  # second element unused, kept for API consistency
...
def _manifold_mixup_batch(
    ...
    loss = -(mixed_targets * log_probs).sum(dim=-1).mean()
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v12: Manifold Mixup (probabilistic)
...
def _train_single(  # noqa: PLR0913
    ...
    *,
    ...
    if jvs_utterances:
        ...
        for epoch in range(1, stage1_epochs + 1):
            loss, acc = _train_epoch(
                ...
                scaler,
                label_smoothing=label_smoothing,
                mixup_alpha=0.0,  # No mixup in pre-training
            )
    ...
    mixup_info = f", Mixup a={mixup_alpha}" if mixup_alpha > 0 else ""
    print(
        f"  [Stage 2] Fine-tuning ({stage2_epochs} ep, "
        f"sched={sched_name}{sam_info}{mixup_info})"
    )
...
def _kfold_split(
    ...
    for fold in range(n_folds):
        ...
        val_utts = [u for i, u in enumerate(utterances) if i in val_idx]
        folds.append((train_utts, val_utts))
...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v12")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of CV folds (0=no CV, use fixed 90/10 split)",
    )
    parser.add_argument(
        "--no-cv",
        action="store_true",
        help="Skip cross-validation, use fixed 90/10 split",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v12 (overfitting reduction)")
    print(f"  Arch: BiLSTM({args.num_layers}) + SelfAttn(4h)")
    print(f"  Dims: embed={args.embed_dim}, hidden={args.hidden_dim}")
    ...
    print(f"  WeightDecay: {args.weight_decay}, Dropout: {args.dropout}")
    print(f"  CV: {'off' if args.no_cv else f'{args.n_folds}-fold'}")
    print("=" * 65)
    ...
    # ── Cross-validation or fixed split ──
    use_cv = not args.no_cv and args.n_folds > 1

    if use_cv:
        print(f"\n{'=' * 65}")
        print(f"[Phase 1] {args.n_folds}-fold Cross-Validation (seed 0 only)")
        print(f"{'=' * 65}")

        folds = _kfold_split(jsut_utterances, n_folds=args.n_folds, seed=42)
        cv_accs: list[float] = []

        for fold_idx, (fold_train, fold_val) in enumerate(folds):
            if corpus_utterances:
                fold_train = fold_train + corpus_utterances  # noqa: PLW2901
            ...
            cv_accs.append(acc)
            print(f"  Fold {fold_idx + 1} result: {acc * 100:.2f}%")

        cv_mean = np.mean(cv_accs)
        cv_std = np.std(cv_accs)
        print(f"\n{'=' * 65}")
        print(f"[CV Result] {cv_mean * 100:.2f}% +/- {cv_std * 100:.2f}%")
        print(f"  Per-fold: {', '.join(f'{a * 100:.2f}' for a in cv_accs)}")
        print(f"{'=' * 65}")

    # ── Multi-seed training on fixed split for final model ──
    print(f"\n{'=' * 65}")
    print("[Phase 2] Multi-seed Best-of-N (fixed 90/10 split)")
    print(f"{'=' * 65}")

    # Fixed val split (seed=42, matching v7+ flow)
    random.seed(42)
    ...
    print(f"  Model params: {n_params_final:,}")
    print(f"{'=' * 65}")
