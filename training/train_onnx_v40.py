r"""kotonoha ONNX accent model trainer (v40).

v39 の仮説検証: teacher feature を morpheme_dropout と feature_noise から保護する。

## v39 の観察から判明した問題
- v39 seed 0 は ep 83 で 78.87%。v38 seed 0 の 80.69% を下回る。
- 原因推定: teacher argmax (dim 13, 14) が morpheme_dropout で 0 に置き換わり、
  feature_noise でガウスノイズを加算されて信号が毀損している。

## v40 の変更点
1. morpheme_dropout 時も teacher feature (dim 13, 14) を保持する。
2. feature_noise を teacher feature にかけない (dim 0-12 のみ)。
3. それ以外は v39 と同一 (15 dim, v24 + v38 argmax)。

## 期待効果
- Student は teacher argmax を確実に参照でき、下限として 80%+ を保証。
- 誤り補正のため student 本来の学習信号も残る (dim 0-12 のみに augment)。

Usage:
  cd training
  uv run python train_onnx_v40.py \
      --teacher-model /mnt/c/GitHub/kotonoha-models/accent_model_v24.onnx \
      --teacher-v38 /mnt/c/GitHub/kotonoha-models/accent_model_v38.onnx
"""
...
FEATURE_DIM = 15  # v39: 14 -> 15 (dim 13 = v24 argmax/20, dim 14 = v38 argmax/20)
...
class _AccentDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        teacher_preds_v38: list[np.ndarray] | None = None,
    ) -> None:
        """Build dataset from parsed utterance dicts.

        v39: teacher_preds (v24 argmax) に加え teacher_preds_v38 (v38 argmax) を
        それぞれ dim 13, 14 に埋め込む。None の場合は 0。
        """
        ...
        tpreds = teacher_preds or [None] * len(utterances)
        tpreds38 = teacher_preds_v38 or [None] * len(utterances)
        for utt, tpred, tpred38 in zip(utterances, tpreds, tpreds38, strict=True):
            ...
            for i, m in enumerate(morphemes):
                ...
                tp38_val = (
                    float(tpred38[i]) / 20.0
                    if (tpred38 is not None and i < len(tpred38))
                    else 0.0
                )
                feat = [*feat_13, tp_val, tp38_val]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.augment:
            features = []
            r_ids = []
            # v40: dim 13, 14 = teacher argmax features. Protect them from
            # morpheme_dropout (zero-out) and feature_noise (gaussian noise)
            # so the student can reliably use them as baseline predictions.
            teacher_feat_start = 13  # dim 13 (v24) and dim 14 (v38) are teachers
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v40: preserve teacher features even when dropping morpheme
                    dropped = [0.0] * FEATURE_DIM
                    for k in range(teacher_feat_start, FEATURE_DIM):
                        dropped[k] = feat[k]
                    ...
                    if self.feature_noise_std > 0:
                        ...
                        # v40: noise only on dim 5..12 (skip teacher features)
                        for j in range(5, teacher_feat_start):
...
class _EmbeddingFrontend(nn.Module):
    ...
    def __init__(
        ...
        self.cont_proj = nn.Linear(10, embed_dim // 4)  # v39: 9->10 cont feats
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    teacher_train_preds_v38: list[np.ndarray] | None = None,
    teacher_val_preds_v38: list[np.ndarray] | None = None,
) -> tuple[float, dict[str, torch.Tensor]]:
    ...
    train_ds = _AccentDataset(
        ...
        teacher_preds_v38=teacher_train_preds_v38,
    )
    val_ds = _AccentDataset(
        val_utts,
        augment=False,
        teacher_preds=teacher_val_preds,
        teacher_preds_v38=teacher_val_preds_v38,
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v40")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--seeds",
        type=str,
        default=",".join(str(i) for i in range(6)),
        help="v39: 6 seeds for initial test (can add more later)",
    )
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v40_states",
        help="v40: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--teacher-cache",
        type=str,
        default="/tmp/v39_teacher_v24_logits.pt",
        help="v39: cache path for precomputed v24 teacher logits",
    )
    parser.add_argument(
        "--teacher-v38",
        type=str,
        default="/mnt/c/GitHub/kotonoha-models/accent_model_v38.onnx",
        help="v39: 2nd teacher ONNX (v38). Empty string to disable dim 14 feature.",
    )
    parser.add_argument(
        "--teacher-v38-cache-train",
        type=str,
        default="/tmp/v39_teacher_v38_train_preds.pt",
        help="v39: cache for v38 train argmax preds",
    )
    parser.add_argument(
        "--teacher-v38-cache-val",
        type=str,
        default="/tmp/v39_teacher_v38_val_preds.pt",
        help="v39: cache for v38 val argmax preds",
    )
    parser.add_argument(
        "--val-split-seed",
        type=int,
        default=0,
        help="v39: default 0 (v31 以降の breakthrough split)",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v40 (15-dim, teacher-feature protected)")
    ...
    if args.teacher_model and Path(args.teacher_model).exists():
        ...
        def _utt_argmax(utt: dict) -> np.ndarray:
            morphemes = utt.get("morphemes", [])
            if not morphemes:
        ...
        # reuse teacher_train_logits if available to avoid re-inference
        if teacher_train_logits is not None and len(teacher_train_logits) == len(
            train_utts_v38
        ):
            teacher_train_preds = [
                (
            ...
            print("\n[v38] Computing train teacher argmax")
            teacher_train_preds = [_utt_argmax(u) for u in train_utts_v38]
        print(f"\n[v38] Computing val teacher argmax ({len(val_utts_v38)} utts)")
        teacher_val_preds = [_utt_argmax(u) for u in val_utts_v38]

    # v39: compute v38 teacher argmax (uses 11-dim features? No: v38 expects 14 dim)
    # Since v38 ONNX expects [seq_len, 14] with v24 argmax in dim 13, we need to
    # embed v24 argmax first then run v38 inference.
    teacher_train_preds_v38: list[np.ndarray] | None = None
    teacher_val_preds_v38: list[np.ndarray] | None = None
    if (
        args.teacher_v38
        and Path(args.teacher_v38).exists()
        and teacher_train_preds is not None
        and teacher_val_preds is not None
    ):
        cache_train_path = Path(args.teacher_v38_cache_train)
        cache_val_path = Path(args.teacher_v38_cache_val)

        if cache_train_path.exists() and cache_val_path.exists():
            print(f"\nLoading cached v38 teacher argmax: {cache_train_path}")
            cached_train = torch.load(cache_train_path, weights_only=False)
            cached_val = torch.load(cache_val_path, weights_only=False)
            # Sanity check: re-build splits
            random.seed(args.val_split_seed)
            indices_chk = list(range(len(jsut_utterances)))
            random.shuffle(indices_chk)
            val_size_chk = int(len(indices_chk) * 0.1)
            val_idx_chk = set(indices_chk[:val_size_chk])
            train_utts_chk = [
                u for i2, u in enumerate(jsut_utterances) if i2 not in val_idx_chk
            ]
            if corpus_utterances:
                train_utts_chk = train_utts_chk + corpus_utterances
            val_utts_chk = [
                u for i2, u in enumerate(jsut_utterances) if i2 in val_idx_chk
            ]
            if len(cached_train) == len(train_utts_chk) and len(cached_val) == len(
                val_utts_chk
            ):
                teacher_train_preds_v38 = cached_train
                teacher_val_preds_v38 = cached_val
                print(
                    f"  train={len(teacher_train_preds_v38)} "
                    f"val={len(teacher_val_preds_v38)}"
                )
            else:
                print(
                    f"  cache size mismatch, recomputing "
                    f"({len(cached_train)}/{len(train_utts_chk)} "
                    f"vs {len(cached_val)}/{len(val_utts_chk)})"
                )

        if teacher_train_preds_v38 is None or teacher_val_preds_v38 is None:
            import onnxruntime as ort
            ...
            sess_v38t = ort.InferenceSession(str(args.teacher_v38), providers=providers)
            # v38 expects [seq, 14] = 13-dim base + v24 argmax / 20 in dim 13
            random.seed(args.val_split_seed)
            indices_v38t = list(range(len(jsut_utterances)))
            random.shuffle(indices_v38t)
            val_size_v38t = int(len(indices_v38t) * 0.1)
            val_idx_v38t = set(indices_v38t[:val_size_v38t])
            train_utts_v38t = [
                u for i2, u in enumerate(jsut_utterances) if i2 not in val_idx_v38t
            ]
            if corpus_utterances:
                train_utts_v38t = train_utts_v38t + corpus_utterances
            val_utts_v38t = [
                u for i2, u in enumerate(jsut_utterances) if i2 in val_idx_v38t
            ]

            def _utt_argmax_v38(utt: dict, v24_pred: np.ndarray) -> np.ndarray:
                morphemes = utt.get("morphemes", [])
                if not morphemes:
                    return np.zeros(0, dtype=np.int32)
                n = len(morphemes)
                ...
                # attach v24 argmax as dim 13 (0..12 base + 13 = v24/20.0)
                base = feats[:, :13]  # [seq, 13]
                v24_col = (
                    v24_pred.astype(np.float32).reshape(-1, 1) / 20.0
                    if len(v24_pred) > 0
                    else np.zeros((0, 1), dtype=np.float32)
                )
                feats14 = np.concatenate([base, v24_col], axis=1)
                logits = sess_v38t.run(None, {"input": feats14})[0]
                return logits.argmax(-1).astype(np.int32)

            print(
                f"\n[v39] Computing v38 teacher argmax "
                f"(train={len(train_utts_v38t)}, val={len(val_utts_v38t)})"
            )
            teacher_train_preds_v38 = [
                _utt_argmax_v38(u, p)
                for u, p in zip(train_utts_v38t, teacher_train_preds, strict=True)
            ]
            teacher_val_preds_v38 = [
                _utt_argmax_v38(u, p)
                for u, p in zip(val_utts_v38t, teacher_val_preds, strict=True)
            ]
            cache_train_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save(teacher_train_preds_v38, cache_train_path)
            torch.save(teacher_val_preds_v38, cache_val_path)
            print(f"  cached -> {cache_train_path.name}, {cache_val_path.name}")
    ...
    for i, seed in enumerate(seeds):
        ...
        if args.resume and state_path.exists():
            ...
            acc, state = _train_single(
                ...
                teacher_train_preds_v38=teacher_train_preds_v38,
                teacher_val_preds_v38=teacher_val_preds_v38,
            )
