"""kotonoha ONNX accent model trainer (v17).

v13ベースに Self-Training (JVSデータフィルタリング) を追加した学習パイプライン:

データ量の拡大 (Self-Training):
- v13 ONNX モデルを使って JVS 事前学習データのアクセント型を予測
- モデル予測と JVS ラベルの一致率が閾値以上の発話のみ Stage 2 に混合
- ノイジーな JVS ラベルをフィルタリングし、高品質なデータのみ追加
- 訓練データの実質的な増量 (JSUT 5000 + corpus 420 + filtered JVS ~1500-2500)
...
Usage:
  ...
  cd training
  uv run python train_onnx_v17.py                           # default: 24 seeds
  uv run python train_onnx_v17.py --seeds 0,1,2             # 3 seeds
  uv run python train_onnx_v17.py --jvs-filter-threshold 0.9  # 厳しいフィルタ
  uv run python train_onnx_v17.py --no-jvs-filter            # JVSフィルタなし
"""
...
# ── JVS data filtering with ONNX model ─────────────────────────────


def _filter_jvs_with_onnx(
    jvs_utterances: list[dict],
    onnx_model_path: str,
    threshold: float = 0.8,
) -> list[dict]:
    """v13 ONNXモデルを使ってJVSデータをフィルタリングする.

    各JVS発話に対してONNX推論を実行し、予測アクセント型とJVSラベルの
    一致率が閾値以上の発話のみを返す。
    ...
    try:
        ...
        print("  WARNING: onnxruntime not available, skipping JVS filter")
        return []
    ...
    for utt in jvs_utterances:
        ...
        agreement = (preds == labels_array).mean()
        total_morphemes += n
    ...
    overall_acc = matched_morphemes / max(total_morphemes, 1)
    print(
        f"  JVS filter: {len(filtered)}/{len(jvs_utterances)} utterances kept "
        f"(threshold={threshold}, model_acc={overall_acc:.4f})"
    )
    return filtered
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v13: Add Gaussian noise to continuous features (indices 5-10)
...
class AccentModel(nn.Module):
    ...
    v13: v11のアーキテクチャ (hidden=256, 3層LSTM) を維持し、
    ...
    def forward(
        ...
        *,
        ...
        # v13: Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v13: Manifold Mixup (in SAM perturbed pass)
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v13: Manifold Mixup (probabilistic)
                if mixup_alpha > 0 and random.random() < mixup_prob:
                    mixup_loss = _manifold_mixup_batch(
                        model,
                        features,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v17")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--seeds",
        type=str,
        default=",".join(str(i) for i in range(24)),
    ...
    parser.add_argument(
        "--teacher-model",
        type=Path,
        default=Path("/mnt/c/GitHub/kotonoha-models/accent_model_v13.onnx"),
        help="ONNX model for JVS data filtering",
    )
    parser.add_argument(
        "--jvs-filter-threshold",
        type=float,
        default=0.8,
        help="Min agreement ratio to keep JVS utterance (0-1)",
    )
    parser.add_argument(
        "--no-jvs-filter",
        action="store_true",
        help="Skip JVS filtering, don't mix JVS into Stage 2",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v17 (v13 + JVS self-training)")
    ...
    # v17: Filter JVS data with teacher model and mix into Stage 2
    filtered_jvs: list[dict] = []
    if not args.no_jvs_filter and jvs_utterances and args.teacher_model.exists():
        print(f"\nFiltering JVS with teacher: {args.teacher_model}")
        filtered_jvs = _filter_jvs_with_onnx(
            jvs_utterances,
            str(args.teacher_model),
            threshold=args.jvs_filter_threshold,
        )

    print(f"\n  JSUT: {len(jsut_utterances)} utterances")
    if corpus_utterances:
        print(f"  Corpus: {len(corpus_utterances)} utterances (mixed into Stage 2)")
    if filtered_jvs:
        print(f"  Filtered JVS: {len(filtered_jvs)} utterances (mixed into Stage 2)")
    ...
    for i, seed in enumerate(seeds):
        ...
        acc, state = _train_single(
            jvs_utterances,
            jsut_utterances,
            corpus_utterances=(corpus_utterances + filtered_jvs) or None,
            seed=seed,
