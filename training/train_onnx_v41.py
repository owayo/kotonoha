r"""kotonoha ONNX accent model trainer (v41).

v38 (80.69%, 14 dim) をベースに JVS 全体を v38 擬似ラベル付きで Stage 2 に追加。

## v39/v40 の失敗からの学び
- v38 argmax を feature に渡す student は teacher 精度 (80.69%) が上限
- train data を拡張して overfitting を抑制し、より強い表現を学習させる

## v41 の変更点
1. corpus: combined + filtered_jvs_v24_t75 に加え、
   **JVS 全体 (3039 発話) を v38 擬似ラベルで追加**
2. Train data: 5754 -> 8793 (+53%)
3. Feature: 14 dim (v38 と同じ、teacher=v24 argmax を dim 13 に)
...
- EMA decay=0.9995, Top-K=15, Last-N=25, SWA enabled

Usage:
  cd training
  uv run python train_onnx_v41.py --seeds 0
"""
...
FEATURE_DIM = 14  # v38: 13 -> 14 (last dim = teacher_pred / 20.0)
...
class _AccentDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        v38: teacher_preds に各発話の argmax 系列 ([seq_len] int) を渡すと
        14 次元目に teacher_pred/20.0 を埋め込む。None なら 0。
        """
        ...
        for utt, tpred in zip(utterances, tpreds, strict=True):
...
class _EmbeddingFrontend(nn.Module):
    ...
    def __init__(
        ...
        self.cont_proj = nn.Linear(9, embed_dim // 4)  # v38: 8->9 cont feats
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v41")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v41_states",
        help="v41: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--teacher-cache",
        type=str,
        default="/tmp/v41_teacher_v24_logits.pt",
        help="v41: cache path for precomputed v24 teacher logits",
    )
    parser.add_argument(
        "--teacher-v38",
        type=str,
        default="/mnt/c/GitHub/kotonoha-models/accent_model_v38.onnx",
        help="v41: v38 ONNX used for JVS pseudo-labeling",
    )
    parser.add_argument(
        "--jvs-pseudo-enable",
        action="store_true",
        default=True,
        help="v41: enable JVS full pseudo-labeling via v38",
    )
    parser.add_argument(
        "--jvs-pseudo-cache",
        type=str,
        default="/tmp/v41_jvs_pseudo.json",
        help="v41: cache path for JVS pseudo-labeled utterances",
    )
    parser.add_argument(
        "--val-split-seed",
        type=int,
        default=0,
        help="v41: default 0 (breakthrough split since v31)",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v41 (14-dim + JVS pseudo-labels)")
    ...
    # v41: Add JVS full pseudo-labeled (via v38) to Stage-2 corpus
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if (
        args.jvs_pseudo_enable
        and jvs_utterances
        and args.teacher_v38
        and Path(args.teacher_v38).exists()
        and Path(args.teacher_model).exists()
    ):
        pseudo_cache = Path(args.jvs_pseudo_cache)
        if pseudo_cache.exists():
            print(f"\nLoading cached JVS pseudo-labeled: {pseudo_cache}")
            with open(pseudo_cache, encoding="utf-8") as f:
                jvs_pseudo = json.load(f)
            print(f"  {len(jvs_pseudo)} utts loaded")
        else:
            print(
                f"\n[v41] Generating JVS pseudo labels "
                f"({len(jvs_utterances)} utts via v24->v38)"
            )
            import onnxruntime as ort
            ...
            sess_v24_pseudo = ort.InferenceSession(
                str(args.teacher_model), providers=providers
            )
            sess_v38_pseudo = ort.InferenceSession(
                str(args.teacher_v38), providers=providers
            )
            jvs_pseudo: list[dict] = []
            for utt in jvs_utterances:
                ...
                feats13 = np.array(
                    [
                        _extract_morpheme_features(m, i / max(n_m - 1, 1))
                        for i, m in enumerate(morphemes)
                    ],
                    dtype=np.float32,
                )
                v24_logits = sess_v24_pseudo.run(None, {"input": feats13[:, :11]})[0]
                v24_arg = v24_logits.argmax(-1).astype(np.int64)
                ...
                v38_logits = sess_v38_pseudo.run(None, {"input": feats14})[0]
                v38_arg = v38_logits.argmax(-1).astype(np.int64)
                pseudo_ms = []
                for i, m in enumerate(morphemes):
                    m2 = dict(m)
                    m2["accent_type"] = int(v38_arg[i])
                    pseudo_ms.append(m2)
                jvs_pseudo.append({"morphemes": pseudo_ms})
            pseudo_cache.parent.mkdir(parents=True, exist_ok=True)
            with open(pseudo_cache, "w", encoding="utf-8") as f:
                json.dump(jvs_pseudo, f, ensure_ascii=False)
            print(f"  cached -> {pseudo_cache}")
        corpus_utterances = corpus_utterances + jvs_pseudo
        print(f"  Corpus with JVS pseudo (v41): {len(corpus_utterances)} utts")
