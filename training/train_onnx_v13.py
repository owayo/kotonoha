"""kotonoha ONNX accent model trainer (v13).

v11ベースに以下の改善を加えた学習パイプライン:

v11のアーキテクチャ (hidden=256, 3層BiLSTM, ~4.7M params) を維持しつつ、
過学習ギャップ縮小のための穏やかな正則化強化:
- Manifold Mixup: LSTM出力レベルで2サンプルを補間 (alpha=0.3, prob=0.3)
- Feature noise: 連続特徴量にガウスノイズ (std=0.02)
- morpheme_dropout微増: 0.1 -> 0.15

v11から継承:
- SAM, R-Drop, Top-5 checkpoint averaging, multi-seed Best-of-N
- Reading dropout (p=0.1)
...
Usage:
  ...
  cd training
  uv run python train_onnx_v13.py                    # default: 24 seeds
  uv run python train_onnx_v13.py --seeds 0,1,2      # 3 seeds
  uv run python train_onnx_v13.py --mixup-alpha 0    # disable Manifold Mixup
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v13")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v13 (v11 + Manifold Mixup)")
