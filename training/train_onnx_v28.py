"""kotonoha ONNX accent model trainer (v28).

v27 (peak 72.11%, KD) に Multi-task learning を追加し 73% 突破を狙う:

## 1. Multi-task learning: phrase boundary prediction
- Auxiliary task: 各 morpheme の「accent_phrase 先頭か」を二値予測
- 共有 BiLSTM+Attn 特徴抽出器 + 2 heads (accent + phrase_start)
- Total loss = L_accent + λ * L_phrase_start (λ=0.3)
- 期待: phrase 境界の明示的学習で accent_type 予測も改善

## 2. 継承 (v27)
- Knowledge Distillation from v24 ONNX teacher (α=0.3, T=2.0)
- Teacher logits precomputed once, reused across seeds

## 3. 継承 (v26)
- corpus: combined_corpus.json (3420) + filtered_jvs_v24_t75.json (834)
- Top-K=15, Last-N=25, SWA enabled
- 標準 CosineAnnealingLR
...
Usage:
  cd training
  uv run python train_onnx_v28.py --phrase-loss-weight 0.3
  uv run python train_onnx_v28.py --seeds 0,1,2 --state-dir /tmp/v28_states
"""
...
class _AccentDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # v27: per-sample teacher logits [seq_len, NUM_CLASSES] or None
        ...
        for utt in utterances:
            ...
            for i, m in enumerate(morphemes):
                ...
                # v28: phrase boundary flag (1 if start of accent phrase)
                is_start = 1 if (prev_acc is None or accent != prev_acc) else 0
                phrase_starts.append(is_start)
                prev_acc = accent
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.augment:
            ...
            out = {
                ...
                "phrase_starts": sample["phrase_starts"],
            }
        else:
            out = dict(sample)
...
def _collate_fn(batch: list[dict]) -> dict:
    ...
    padded_phrase = torch.zeros(len(batch), max_len, dtype=torch.float32)
    ...
    for i, item in enumerate(batch):
        ...
        if "phrase_starts" in item:
            padded_phrase[i, :seq_len] = torch.tensor(
                item["phrase_starts"], dtype=torch.float32
            )
...
class AccentModel(nn.Module):
    ...
    def __init__(
        ...
        # v28: phrase boundary prediction head (binary; returns per-morpheme logit)
    ...
    def forward(
        ...
        *,
        ...
        Args:
            ...
            return_phrase: v28 — if True, also return phrase boundary logits.

        Returns:
            logits (accent) or tuple depending on flags.
        ...
        h_attn = self.self_attn(h, mask=attn_mask)
        h_drop = self.dropout(h_attn)
        accent_logits = self.classifier(h_drop)
        if return_phrase:
            phrase_logits = self.phrase_head(h_drop).squeeze(-1)
            return accent_logits, phrase_logits
...
def _train_epoch(  # noqa: PLR0913
    ...
    for batch in dataloader:
        ...
        phrase_targets: torch.Tensor | None = None
        if phrase_loss_weight > 0 and "phrase_starts" in batch:
            phrase_targets = batch["phrase_starts"].to(device)[mask]

        if sam_rho > 0:
            # ── SAM: gradient at current → perturb → gradient at perturbed ──
            optimizer.zero_grad()
            with torch.amp.autocast("cuda", enabled=use_amp):
                if phrase_targets is not None:
                    logits, phrase_logits = model(
                        features, lengths, r_ids, return_phrase=True
                    )
                    phrase_logits_flat = phrase_logits[mask]
                else:
                    logits = model(features, lengths, r_ids)
                    phrase_logits_flat = None
                ...
                if kd_alpha > 0 and teacher_flat is not None:
                    ...
                    loss = ce_mean
                if phrase_targets is not None and phrase_logits_flat is not None:
                    phrase_loss = F.binary_cross_entropy_with_logits(
                        phrase_logits_flat, phrase_targets
                    )
                    loss = loss + phrase_loss_weight * phrase_loss
                if rdrop_alpha > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                if phrase_targets is not None:
                    logits_p, phrase_logits_p = model(
                        features, lengths, r_ids, return_phrase=True
                    )
                    phrase_logits_p_flat = phrase_logits_p[mask]
                else:
                    logits_p = model(features, lengths, r_ids)
                    phrase_logits_p_flat = None
                ...
                if kd_alpha > 0 and teacher_flat is not None:
                    ...
                    loss_p = ce_p_mean
                if phrase_targets is not None and phrase_logits_p_flat is not None:
                    phrase_loss_p = F.binary_cross_entropy_with_logits(
                        phrase_logits_p_flat, phrase_targets
                    )
                    loss_p = loss_p + phrase_loss_weight * phrase_loss_p
                if rdrop_alpha > 0:
    ...
    *,
    ...
    for epoch in range(1, stage2_epochs + 1):
        tr_loss, tr_acc = _train_epoch(
            ...
            phrase_loss_weight=phrase_loss_weight,
        )
...
def main() -> None:
    ...
    # v28: Multi-task phrase boundary prediction
    parser.add_argument(
        "--phrase-loss-weight",
        type=float,
        default=0.3,
        help="v28: phrase boundary BCE loss weight (0=disabled)",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v28 (KD + phrase boundary MTL)")
    ...
    for i, seed in enumerate(seeds):
        ...
        if args.resume and state_path.exists():
            ...
            acc, state = _train_single(
                ...
                phrase_loss_weight=args.phrase_loss_weight,
            )
