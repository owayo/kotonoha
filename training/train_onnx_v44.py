"""kotonoha ONNX accent model trainer (v24).

v20ベース(combined_corpus.json 3420, 72.20%実績)に codex 推奨3施策を追加
(73%目標):

## C. Extended training + slow LR decay
- stage2_epochs 80 → 120
- scheduler: plateau → cosine (T_max=120, eta_min=1e-6)
- patience 20 → 30
- 狙い: flat region に長く滞在して single ceiling を押し上げる

## A. Top-K 拡大 + greedy soup
- top_k 5 → 10 (best val_acc checkpoint)
- 追加: last N epochs (15) のcheckpointも候補に
- 選択: 単純avgではなく greedy soup (1個ずつ加算、val改善なら採用)
- 狙い: v20-r2 seed 89 で Top-5 avg が +0.42% だったのを拡張

## D. EMA shadow weights
- decay=0.999 で model weights の EMA を並行追跡
- EMA model も greedy soup 候補に含める
- 狙い: SAM/R-Drop のノイズを吸収して近接 checkpoint avg を強化

## 継承 (v20)
- BiLSTM(3層)+SelfAttn, SAM rho=0.05, R-Drop α=0.5, Manifold Mixup α=0.3
- combined_corpus.json (LLM 420 + recombined 2000 + phrase-replaced 1000)
...
Usage:
  ...
  cd training
  uv run python train_onnx_v24.py                    # default: 48 seeds
  uv run python train_onnx_v24.py --seeds 0,1,2      # 3 seeds
  uv run python train_onnx_v24.py --ema-decay 0.999  # default
  uv run python train_onnx_v24.py --no-ema           # disable EMA
  uv run python train_onnx_v24.py --no-greedy-soup   # plain Top-K avg
"""
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    # ── Val split (v44: configurable seed) ──
...
def main() -> None:
    ...
    parser.add_argument("--stage2-epochs", type=int, default=120, help="v24: 80->120")
    parser.add_argument("--stage2-lr", type=float, default=8e-4)
    parser.add_argument("--stage2-patience", type=int, default=30, help="v24: 20->30")
    ...
    parser.add_argument("--top-k", type=int, default=10, help="v24: Top-K 5->10")
    parser.add_argument(
        "--val-split-seed",
        type=int,
        default=0,
        help="v44: default 0 (breakthrough split since v31)",
    )
    parser.add_argument(
        "--scheduler",
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="v24: Last-N epochs checkpoints for greedy soup",
    )
    parser.add_argument(
        "--ema-decay",
        type=float,
        default=0.999,
        help="v24: EMA decay rate",
    )
    parser.add_argument(
        "--no-ema",
        action="store_true",
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v24 (ext train + greedy soup + EMA)")
