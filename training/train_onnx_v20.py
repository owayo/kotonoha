"""kotonoha ONNX accent model trainer (v20).

v13ベースに複合コーパス(3420発話)を追加:

データ拡張 (generate_combined_corpus.py で生成):
- LLMコーパス (420発話) - v6で+1.07%の実績
- アクセント句リコンビネーション (2000発話) - v18で+0.45%の実績
- 単一句置換 (1000発話) - JSUT発話の1句を別の句に置換、文構造保持
...
Usage:
  ...
  cd training
  uv run python train_onnx_v20.py                    # default: 48 seeds
  uv run python train_onnx_v20.py --seeds 0,1,2      # 3 seeds
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v20: Add Gaussian noise to continuous features (indices 5-10)
...
class AccentModel(nn.Module):
    ...
    v20: v11のアーキテクチャ (hidden=256, 3層LSTM) を維持し、
    ...
    def forward(
        ...
        *,
        ...
        # v20: Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v20: Manifold Mixup (in SAM perturbed pass)
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v20: Manifold Mixup (probabilistic)
                if mixup_alpha > 0 and random.random() < mixup_prob:
                    mixup_loss = _manifold_mixup_batch(
                        model,
                        features,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v20")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v20 (v13 + Combined Corpus)")
