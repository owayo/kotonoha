"""kotonoha ONNX accent model trainer (v25).

v24 (72.33%)をベースに、73%突破を狙う施策を追加:

## 1. Teacher-Guided Self-Training (v24 teacher)
- v24 ONNX (72.33%) を teacher として JVS 発話を予測
- 真ラベルと一致率 >= 0.75 の発話のみ抽出 (834発話, filtered_jvs_v24_t75.json)
- v17 (teacher=v13 71.59%, thr=0.8, 603発話) は +0% だったが、
  teacher 精度向上 + 量増で汎化改善を狙う
- combined_corpus (3420) と合わせて +4254発話の追加学習データ

## 2. Extended averaging window
- top_k 10 → 15 (best val_acc checkpoint)
- last_n 15 → 25 (rolling window)
- 候補が増えて greedy soup の最適点探索の自由度向上

## 3. Slightly extended Stage 2
- stage2_epochs 120 → 150
- eta_min 1e-6 → 5e-7 (更にフラット領域に滞在)
- patience 30 → 35

## 4. Tighter EMA decay
- ema_decay 0.999 → 0.9995 (より長期の平均、後半重視)
...
- Per-seed greedy soup from {top-K, last-N, EMA}
...
Usage:
  ...
  cd training
  uv run python train_onnx_v25.py                          # default: 48 seeds
  uv run python train_onnx_v25.py --seeds 0,1,2            # 3 seeds
  uv run python train_onnx_v25.py --extra-corpus-data ""   # disable JVS
  uv run python train_onnx_v25.py --no-greedy-soup         # plain Top-K avg
"""
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    # ── v24 A: Greedy Soup (union of top-K, last-N, EMA) ──
    if use_greedy_soup:
        candidates: list[tuple[float, dict[str, torch.Tensor]]] = []
        seen_ids: set[int] = set()
        for acc, state in top_states + last_states:
...
def main() -> None:
    ...
    parser.add_argument("--stage2-patience", type=int, default=35, help="v25: 30->35")
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v25 (self-training + ext avg)")
