"""kotonoha ONNX accent model trainer (v16).

v13ベースに前後文脈POS特徴量を追加した学習パイプライン:

特徴量の改善 (FEATURE_DIM: 11 → 13):
- prev_pos: 前の形態素の品詞ID (index 11)
- next_pos: 次の形態素の品詞ID (index 12)
- 共有 pos_emb を使用して埋め込み (新パラメータ追加なし)
- アクセント型は前後の品詞に強く依存するため、明示的な文脈特徴量が有効
...
特徴量は nn.rs の extract_features_v3 と完全に一致させること:
...
Usage:
  ...
  cd training
  uv run python train_onnx_v16.py                    # default: 24 seeds
  uv run python train_onnx_v16.py --seeds 0,1,2      # 3 seeds
"""
...
FEATURE_DIM = 13  # v16: 11 base + 2 context (prev_pos, next_pos)
...
def _extract_morpheme_features(
    morpheme: dict,
    position: float,
    prev_morpheme: dict | None = None,
    next_morpheme: dict | None = None,
) -> list[float]:
    """形態素辞書から特徴量ベクトルを抽出する.
    ...
    # v16: Context POS features
    prev_pos_id = POS_VOCAB.get(prev_morpheme["pos"], 0) if prev_morpheme else 0
    next_pos_id = POS_VOCAB.get(next_morpheme["pos"], 0) if next_morpheme else 0
        ...
        *,
        ...
        for utt in utterances:
            ...
            for i, m in enumerate(morphemes):
                ...
                next_m = morphemes[i + 1] if i < n - 1 else None
                feat = _extract_morpheme_features(m, position, prev_m, next_m)
...
class _EmbeddingFrontend(nn.Module):
    ...
    def __init__(
        ...
        # v16: Context POS (prev_pos + next_pos) share pos_emb
        ctx_emb_dim = 2 * (embed_dim // 4)
        input_dim = cat_emb_dim + ctx_emb_dim + embed_dim // 4 + embed_dim // 4
    ...
    def forward(
        ...
        """
        cat_feats = x[:, :, :5].long()
        cont_feats = x[:, :, 5:11]  # v16: only original 6 continuous features
        ctx_feats = x[:, :, 11:13].long()  # v16: context POS (prev, next)
        ...
        # v16: Context POS embeddings (shared with pos_emb)
        prev_pos_ids = ctx_feats[:, :, 0].clamp(0, NUM_POS - 1)
        next_pos_ids = ctx_feats[:, :, 1].clamp(0, NUM_POS - 1)
        e_prev_pos = self.pos_emb(prev_pos_ids)
        e_next_pos = self.pos_emb(next_pos_ids)
        ...
        h = torch.cat(
            [e_pos, e_pd1, e_pd2, e_ct, e_cf, e_prev_pos, e_next_pos, e_kana, e_cont],
            dim=-1,
        )
        h = self.input_norm(h)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v16")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v16 (v13 + context POS features)")
