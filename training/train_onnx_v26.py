"""kotonoha ONNX accent model trainer (v26).

v25 (peak 72.25%, mean 71.38%) をベースに、73%突破を狙う新戦略:

## 1. Cosine Warm Restarts (checkpoint diversity)
- scheduler: CosineAnnealingWarmRestarts T_0=50, T_mult=1 (150ep = 3 cycles)
- 各 restart 直前の checkpoint は LR が最小点で flat region、greedy soup 候補に追加
- v14 での単独失敗は R-Drop 0.7 が原因、v26 は R-Drop 0.5 + greedy soup で併用

## 2. SWA (Stochastic Weight Averaging)
- Stage 2 後半 (epoch >= stage2_epochs * 0.6) の weights の単純平均を追跡
- Top-K, last-N, EMA, SWA の4系統が greedy soup 候補
- 多様な平均化方法を pool することで greedy soup の最適点到達を強化

## 3. Per-seed state persistence (disk save)
- 各 seed の best_state を `state_{seed}.pt` として disk 保存
- 中断 or resume 可能
- 終了後に cross-seed greedy soup を別スクリプトで試験可能

## 4. Reduced seeds + longer patience
- seeds 48 → 32 (時間短縮)
- patience 35 → 50 (warm restart 許容)

## 継承 (v25)
- self-training filtered_jvs_v24_t75.json (834 発話)
- combined_corpus.json (3420)
- top_k=15, last_n=25
- SAM rho=0.05, R-Drop α=0.5, Manifold Mixup α=0.3
...
Usage:
  cd training
  uv run python train_onnx_v26.py
  uv run python train_onnx_v26.py --seeds 0,1,2
  uv run python train_onnx_v26.py --state-dir /tmp/v26_states
  uv run python train_onnx_v26.py --resume
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v24: Add Gaussian noise to continuous features (indices 5-10)
...
def main() -> None:
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v26 (warm restarts + SWA + state save)")
