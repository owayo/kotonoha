"""kotonoha ONNX accent model trainer (v23).

v20ベースにコーパス品質改善 + 微調整:

改善点 (v20からの差分):
- コーパス長文化: recomb 2-5 phrases → 3-6 phrases (avg 11.4→13.0 morphemes)
- コーパス多様化: 2つの乱数seed (42+77) で生成
- morpheme_dropout: 0.15→0.18 (過学習耐性微増)
- label_smoothing: 0.10→0.08 (予測シャープ化)
- 総量は3420で維持 (v21の4920増量が逆効果だった教訓)
- training procedure は v20 と同一 (σ維持のため変更なし)

v13から継承:
- Manifold Mixup, Feature noise
...
Usage:
  ...
  cd training
  uv run python train_onnx_v23.py                    # default: 48 seeds
  uv run python train_onnx_v23.py --seeds 0,1,2      # 3 seeds
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v23: Add Gaussian noise to continuous features (indices 5-10)
...
class AccentModel(nn.Module):
    ...
    v23: v11のアーキテクチャ (hidden=256, 3層LSTM) を維持し、
    ...
    def forward(
        ...
        *,
        ...
        # v23: Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v23: Manifold Mixup (in SAM perturbed pass)
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v23: Manifold Mixup (probabilistic)
                if mixup_alpha > 0 and random.random() < mixup_prob:
                    mixup_loss = _manifold_mixup_batch(
                        model,
                        features,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v23")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.18,
        help="Prob of dropping morpheme features during training (v23: 0.15→0.18)",
    )
    parser.add_argument(
        "--feature-noise-std",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v23 (v13 + Combined Corpus v3)")
