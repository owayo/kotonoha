"""kotonoha ONNX accent model trainer (v22).

v20ベース (combined_corpus.json 3420発話) に codex 推奨の2段階 fine-tuning と
reading_dropout schedule を追加 (72%突破を狙う):

## Phase 1: mixed corpus domain-adaptive
- duration: 最大60エポック (early stopping patience=12)
- corpus: combined_corpus.json (JSUT + LLM + recombined + phrase-replaced)
- reading_dropout: 0.10 (ep 1-10) → 線形 0.25 (ep 11-40) → 0.25 (ep 41+)
- optimizer: AdamW lr=8e-4, ReduceLROnPlateau
- augmentation: SAM rho=0.05, R-Drop α=0.5, Manifold Mixup α=0.3 prob=0.3

## Phase 2: JSUT-only fine-tuning
- duration: 最大20エポック (early stopping patience=8)
- 起点: Phase 1 の best val accuracy チェックポイント
- corpus: JSUT のみ (combined_corpus なし)
- lr reset: 4e-4, ReduceLROnPlateau(factor=0.5, patience=3)
- reading_dropout: 0.20 固定 (Phase 1 の 0.25 から少し戻す)
- augmentation: SAM/R-Drop 維持, Mixup 弱体化 (α=0.2 prob=0.15)

## 設計意図 (codex と協議)
- v21 の失敗は corpus 増量 (3420→4920) による JSUT 分布からの乖離
- Phase 1 で corpus の多様性を吸収, Phase 2 で JSUT に再適応
- reading_dropout を後半上げて train-ONNX inference mismatch を軽減
...
Usage:
  ...
  cd training
  uv run python train_onnx_v22.py                    # default: 48 seeds
  uv run python train_onnx_v22.py --seeds 0,1,2      # 3 seeds
  uv run python train_onnx_v22.py --phase2-epochs 0  # disable Phase 2
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v22: Add Gaussian noise to continuous features (indices 5-10)
...
class AccentModel(nn.Module):
    ...
    v22: v11のアーキテクチャ (hidden=256, 3層LSTM) を維持し、
    ...
    def forward(
        ...
        *,
        ...
        # v22: Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v22: Manifold Mixup (in SAM perturbed pass)
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v22: Manifold Mixup (probabilistic)
                if mixup_alpha > 0 and random.random() < mixup_prob:
                    mixup_loss = _manifold_mixup_batch(
                        model,
                        features,
...
def _compute_reading_dropout(
    ...
    *,
    ...
    """v22: Phase 1 の reading_dropout スケジュール.

    - ep 1 .. ramp_start: rd_start 固定
    - ep ramp_start+1 .. ramp_end: 線形増加 rd_start → rd_end
    - ep ramp_end 以降: rd_end 固定

    Returns:
        Current epoch の reading_dropout 値.
    ...
    if epoch >= ramp_end:
        return rd_end
    ratio = (epoch - ramp_start) / max(ramp_end - ramp_start, 1)
    return rd_start + (rd_end - rd_start) * ratio
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    phase2_lr: float = 4e-4,
    phase2_reading_dropout: float = 0.20,
    phase2_mixup_alpha: float = 0.2,
    phase2_mixup_prob: float = 0.15,
    rd_start: float = 0.10,
    rd_end: float = 0.25,
    rd_ramp_start: int = 10,
    rd_ramp_end: int = 40,
    ...
    jsut_train_utts = [u for i, u in enumerate(jsut_utterances) if i not in val_idx]
    val_utts = [u for i, u in enumerate(jsut_utterances) if i in val_idx]

    phase1_train_utts = list(jsut_train_utts)
    if corpus_utterances:
        phase1_train_utts = phase1_train_utts + corpus_utterances
    ...
    # Shared tracking across phases
    best_val_acc = 0.0
    best_state: dict | None = None
    top_states: list[tuple[float, dict]] = []

    def _update_top_k(acc: float, state: dict) -> None:
        nonlocal top_states
        if len(top_states) < top_k:
            top_states.append((acc, state))
            top_states.sort(key=lambda x: x[0])
        elif acc > top_states[0][0]:
            top_states[0] = (acc, state)
            top_states.sort(key=lambda x: x[0])

    # ── Stage 2 Phase 1: mixed corpus + reading_dropout schedule ──
    sam_info = f", SAM rho={sam_rho}" if sam_rho > 0 else ""
    print(
        f"  [Phase 1] mixed corpus ({stage2_epochs} ep, "
        f"patience={stage2_patience}, rd {rd_start}->{rd_end}{sam_info})"
    )

    phase1_ds = _AccentDataset(
        phase1_train_utts,
        augment=True,
        morpheme_dropout=morpheme_dropout,
        feature_noise_std=feature_noise_std,
    ...
    # Phase 1 uses plateau scheduler regardless of use_cosine for compatibility
    ...
    phase1_best_acc = 0.0
    phase1_best_state: dict | None = None
    no_improve = 0

    for epoch in range(1, stage2_epochs + 1):
        # v22: schedule reading_dropout during Phase 1
        current_rd = _compute_reading_dropout(
            epoch,
            rd_start=rd_start,
            rd_end=rd_end,
            ramp_start=rd_ramp_start,
            ramp_end=rd_ramp_end,
        )
        model.reading_dropout = current_rd
        ...
        marker = ""
        if va_acc > phase1_best_acc:
            phase1_best_acc = va_acc
            phase1_best_state = {
        ...
        _update_top_k(va_acc, state_copy)

        if epoch % 5 == 0 or marker or epoch <= 3:
            gap = tr_acc - va_acc
            print(
                f"    P1 ep {epoch:2d}: va={va_acc:.4f} tr={tr_acc:.4f} "
                f"gap={gap:.4f} lr={lr:.6f} rd={current_rd:.2f}{marker}"
            )

        if no_improve >= stage2_patience:
            print(f"    Early stop at Phase 1 ep {epoch}")
            break

    print(f"  Phase 1 best: {phase1_best_acc * 100:.2f}%")

    # ── Stage 2 Phase 2: JSUT-only fine-tuning from Phase 1 best ──
    if phase2_epochs > 0 and phase1_best_state is not None:
        print(
            f"  [Phase 2] JSUT-only ({phase2_epochs} ep, "
            f"patience={phase2_patience}, lr={phase2_lr}, "
            f"rd={phase2_reading_dropout})"
        )
        model.load_state_dict(phase1_best_state)
        model.to(device)
        model.reading_dropout = phase2_reading_dropout

        phase2_ds = _AccentDataset(
            jsut_train_utts,
            augment=True,
            morpheme_dropout=morpheme_dropout,
            feature_noise_std=feature_noise_std,
        ...
        # Fresh optimizer + scheduler for Phase 2 (LR reset)
        optimizer = torch.optim.AdamW(
            model.parameters(),
            lr=phase2_lr,
            weight_decay=weight_decay,
        ...
        no_improve = 0
        phase2_start_best = best_val_acc
        ...
        for epoch in range(1, phase2_epochs + 1):
            ...
            if epoch % 3 == 0 or marker or epoch <= 3:
                gap = tr_acc - va_acc
                print(
                    f"    P2 ep {epoch:2d}: va={va_acc:.4f} tr={tr_acc:.4f} "
                    f"gap={gap:.4f} lr={lr:.6f}{marker}"
                )

            if no_improve >= phase2_patience:
                print(f"    Early stop at Phase 2 ep {epoch}")
                break

        delta = (best_val_acc - phase2_start_best) * 100
        print(f"  Phase 2 delta: {delta:+.2f}%")

    print(f"  Best single: {best_val_acc * 100:.2f}%")

    # ── Top-K Checkpoint Averaging (across both phases) ──
    if len(top_states) >= 2:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v22")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Phase 1 max epochs (mixed corpus)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Phase 1 early stopping patience",
    )
    ...
    # v22: Phase 2 (JSUT-only fine-tuning) args
    parser.add_argument(
        "--phase2-epochs",
        type=int,
        default=20,
        help="Phase 2 max epochs (0=disabled)",
    )
    parser.add_argument(
        "--phase2-patience",
        type=int,
        default=8,
        help="Phase 2 early stopping patience",
    )
    parser.add_argument(
        "--phase2-lr",
        type=float,
        default=4e-4,
        help="Phase 2 LR (reset from Phase 1)",
    )
    parser.add_argument(
        "--phase2-reading-dropout",
        type=float,
        default=0.20,
        help="Phase 2 reading_dropout (fixed)",
    )
    parser.add_argument(
        "--phase2-mixup-alpha",
        type=float,
        default=0.2,
        help="Phase 2 mixup alpha (weakened)",
    )
    parser.add_argument(
        "--phase2-mixup-prob",
        type=float,
        default=0.15,
        help="Phase 2 mixup prob (weakened)",
    )
    # v22: reading_dropout schedule args (Phase 1)
    parser.add_argument(
        "--mixup-prob",
        type=float,
        default=0.3,
        help="Phase 1 mixup probability",
    )
    parser.add_argument(
        "--rd-start",
        type=float,
        default=0.10,
        help="Phase 1 reading_dropout start (ep 1..ramp_start)",
    )
    parser.add_argument(
        "--rd-end",
        type=float,
        default=0.25,
        help="Phase 1 reading_dropout end (ep ramp_end..)",
    )
    parser.add_argument(
        "--rd-ramp-start",
        type=int,
        default=10,
        help="Phase 1 reading_dropout ramp start epoch",
    )
    parser.add_argument(
        "--rd-ramp-end",
        type=int,
        default=40,
        help="Phase 1 reading_dropout ramp end epoch",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v22 (2-phase fine-tune + rd schedule)")
    print(f"  Arch: BiLSTM({args.num_layers}) + SelfAttn(4h), hidden={args.hidden_dim}")
    print(f"  Seeds: {len(seeds)} seeds (best-of-N)")
    print(f"  R-Drop: {args.rdrop_alpha}, Top-K: {args.top_k}, SAM: {args.sam_rho}")
    print(f"  MorphDrop: {args.morpheme_dropout}, FeatNoise: {args.feature_noise_std}")
    print(
        f"  P1: {args.stage2_epochs}ep patience={args.stage2_patience} "
        f"lr={args.stage2_lr} mixup={args.mixup_alpha}/{args.mixup_prob}"
    )
    print(
        f"      rd {args.rd_start}->{args.rd_end} "
        f"ramp [{args.rd_ramp_start}-{args.rd_ramp_end}]"
    )
    if args.phase2_epochs > 0:
        print(
            f"  P2: {args.phase2_epochs}ep patience={args.phase2_patience} "
            f"lr={args.phase2_lr} rd={args.phase2_reading_dropout} "
            f"mixup={args.phase2_mixup_alpha}/{args.phase2_mixup_prob}"
        )
    else:
        print("  P2: disabled")
    print("=" * 65)
    ...
    for i, seed in enumerate(seeds):
        ...
        acc, state = _train_single(
            ...
            phase2_reading_dropout=args.phase2_reading_dropout,
            phase2_mixup_alpha=args.phase2_mixup_alpha,
            phase2_mixup_prob=args.phase2_mixup_prob,
            rd_start=args.rd_start,
            rd_end=args.rd_end,
            rd_ramp_start=args.rd_ramp_start,
            rd_ramp_end=args.rd_ramp_end,
        )
