...
FEATURE_DIM = 14 + 768  # v50: v38 features + BERT embedding (768 dim)
ACCENT_FEAT_DIM = 14  # legacy accent features (dims 0-13)
BERT_DIM = 768
...
class _AccentDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        bert_embs: list[np.ndarray] | None = None,
    ) -> None:
        """Build dataset from parsed utterance dicts.

        v50: bert_embs に各発話の BERT embedding ([seq_len, 768]) を渡すと
        feature の dim 14 以降に concat される。None なら 0 padding。
        """
        ...
        bertes = bert_embs or [None] * len(utterances)
        for utt, tpred, bert_emb in zip(utterances, tpreds, bertes, strict=True):
            ...
            for i, m in enumerate(morphemes):
                ...
                bert_vec = (
                    bert_emb[i].tolist()
                    if (bert_emb is not None and i < len(bert_emb))
                    else [0.0] * BERT_DIM
                )
                feat = [*feat_13, tp_val, *bert_vec]
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    if self.feature_noise_std > 0:
                        ...
                        # v50: noise only on accent feature continuous dims
                        # (skip BERT 768 dims to keep BERT signal clean)
                        for j in range(5, ACCENT_FEAT_DIM):
...
class _EmbeddingFrontend(nn.Module):
    ...
    def __init__(
        ...
        # cont_feats: dims 5..13 (9 numeric values:
        # mora, hashes, position, dict, teacher_pred)
        self.cont_proj = nn.Linear(9, embed_dim // 4)
        # v50: BERT projection (768 -> embed_dim)
    ...
    def forward(
        ...
        """
        cat_feats = x[:, :, :5].long()
        # cont_feats: dims 5..13 (9 numeric: mora, r_hash, first/last char,
        # position, dict_acc, head2/tail2 hashes, teacher_pred)
        cont_feats = x[:, :, 5:ACCENT_FEAT_DIM]
        bert_feats = x[:, :, ACCENT_FEAT_DIM:]
        ...
        e_bert = self.bert_dropout(self.bert_proj(bert_feats))

        h = torch.cat(
            [e_pos, e_pd1, e_pd2, e_ct, e_cf, e_kana, e_cont, e_bert],
            dim=-1,
        )
        h = self.input_norm(h)
...
def _train_single(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    bert_jvs: list[np.ndarray] | None = None,
    bert_jsut: list[np.ndarray] | None = None,
    bert_corpus: list[np.ndarray] | None = None,
) -> tuple[float, dict[str, torch.Tensor]]:
    ...
    # v50: split BERT embeddings consistently
    bert_train_list: list[np.ndarray] | None = None
    bert_val_list: list[np.ndarray] | None = None
    if bert_jsut is not None:
        bert_train_list = [b for i, b in enumerate(bert_jsut) if i not in val_idx]
        bert_val_list = [b for i, b in enumerate(bert_jsut) if i in val_idx]

    if corpus_utterances:
        train_utts = train_utts + corpus_utterances
        if bert_train_list is not None and bert_corpus is not None:
            bert_train_list = bert_train_list + list(bert_corpus)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v51")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--seeds",
        type=str,
        default=",".join(str(i) for i in range(3)),
        help="v50: start with 3 seeds for BERT experiment",
    )
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v51_states",
        help="v50: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--teacher-cache",
        type=str,
        default="/tmp/v51_teacher_v24_logits.pt",
        help="v50: cache path for precomputed v24 teacher logits",
    )
    parser.add_argument(
        "--val-split-seed",
        type=int,
        default=0,
        help="v50: default 0 (breakthrough split since v31)",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    # v50: BERT embedding caches (from precompute_bert_embeddings.py)
    parser.add_argument(
        "--bert-cache-jvs",
        type=str,
        default="/tmp/bert_v2_emb_jvs.pt",
    )
    parser.add_argument(
        "--bert-cache-jsut",
        type=str,
        default="/tmp/bert_v2_emb_jsut.pt",
    )
    parser.add_argument(
        "--bert-cache-corpus",
        type=str,
        default="/tmp/bert_v2_emb_corpus.pt",
    )
    parser.add_argument(
        "--bert-cache-filtered-jvs",
        type=str,
        default="/tmp/bert_v2_emb_filtered_jvs.pt",
    )
    ...
    print("=" * 65)
    print(
        "kotonoha accent model trainer v51 (14-dim + 768-dim char-BERT context embedding)"
    )
    ...
    # v50: Load BERT embeddings (precomputed by precompute_bert_embeddings.py)
    bert_jvs: list[np.ndarray] | None = None
    bert_jsut: list[np.ndarray] | None = None
    bert_corpus: list[np.ndarray] | None = None
    bert_cache_jvs = Path(args.bert_cache_jvs)
    bert_cache_jsut = Path(args.bert_cache_jsut)
    bert_cache_corpus = Path(args.bert_cache_corpus)
    bert_cache_filtered = Path(args.bert_cache_filtered_jvs)
    if bert_cache_jvs.exists():
        bert_jvs = torch.load(bert_cache_jvs, weights_only=False)
        print(f"loaded BERT JVS: {len(bert_jvs)} utts from {bert_cache_jvs}")
    if bert_cache_jsut.exists():
        bert_jsut = torch.load(bert_cache_jsut, weights_only=False)
        print(f"loaded BERT JSUT: {len(bert_jsut)} utts from {bert_cache_jsut}")
    if bert_cache_corpus.exists():
        bert_corpus = list(torch.load(bert_cache_corpus, weights_only=False))
        print(f"loaded BERT corpus: {len(bert_corpus)} utts from {bert_cache_corpus}")
    if bert_cache_filtered.exists():
        bert_filt = torch.load(bert_cache_filtered, weights_only=False)
        print(
            f"loaded BERT filtered_jvs: {len(bert_filt)} utts "
            f"from {bert_cache_filtered}"
        )
        if bert_corpus is None:
            bert_corpus = list(bert_filt)
        else:
            bert_corpus = bert_corpus + list(bert_filt)
    if bert_corpus is not None:
        print(f"  combined BERT corpus: {len(bert_corpus)} utts")
