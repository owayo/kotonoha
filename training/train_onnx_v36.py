...
FEATURE_DIM = 13  # v33: 11 -> 13 (add reading_head2_hash, reading_tail2_hash)
...
class _EmbeddingFrontend(nn.Module):
    ...
    def __init__(
        ...
        self.cont_proj = nn.Linear(8, embed_dim // 4)  # v33: 6->8 cont feats
...
def _neighbor_ce(
    ...
    """Neighbor-aware label smoothing cross-entropy.

    target y に質量 (1 - alpha - spread), neighbor y±1 に spread/2 ずつ,
    残り alpha をクラス全体に uniform 分配.

    logits: [N, C], targets: [N] long.

    Returns:
        [N] per-sample loss (no reduction).

    """
    num_classes = logits.size(-1)
    log_soft = F.log_softmax(logits, dim=-1)
    with torch.no_grad():
        t = torch.full_like(log_soft, alpha / num_classes)
        t.scatter_(1, targets.unsqueeze(1), 1.0 - alpha - neighbor_spread)
        upper = (targets + 1).clamp(max=num_classes - 1).unsqueeze(1)
        t.scatter_add_(
            1, upper, torch.full_like(upper, neighbor_spread / 2, dtype=t.dtype)
        )
        lower = (targets - 1).clamp(min=0).unsqueeze(1)
        t.scatter_add_(
            1, lower, torch.full_like(lower, neighbor_spread / 2, dtype=t.dtype)
        )
    return -(t * log_soft).sum(dim=-1)
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                if neighbor_spread > 0:
                    ce = _neighbor_ce(
                        logits_flat, labels_flat, label_smoothing, neighbor_spread
                    )
                else:
                    ce = F.cross_entropy(
                        logits_flat,
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                if neighbor_spread > 0:
                    ce_p = _neighbor_ce(
                        logits_p_flat, labels_flat, label_smoothing, neighbor_spread
                    )
                else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="v36: mass on target y±1 (neighbor-aware smoothing; 0=off)",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v36 (neighbor-aware smoothing)")
        ...
        if not teacher_path.exists():
            ...
            if teacher_train_logits is None:
                ...
                for j, utt in enumerate(train_utts_for_kd):
                    ...
                    # v33: teacher ONNX は 11 dim 入力を要求する。
                    # student は 13 dim だが、teacher 推論は旧 11 dim で行う。
    ...
    for i, seed in enumerate(seeds):
        ...
        if args.resume and state_path.exists():
            ...
            acc, state = _train_single(
                ...
                neighbor_spread=args.neighbor_spread,
            )
