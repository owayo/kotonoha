...
class AccentModel(nn.Module):
    ...
    def __init__(
        ...
        # v37: CRF layer (学習時のみ使う。ONNX export には含まない)
        ...
        if sam_rho > 0:
            ...
            if crf_weight > 0:
                with torch.amp.autocast("cuda", enabled=False):
                    labels_crf = labels.masked_fill(~mask, 0)
                    crf_nll = -model.crf(
                        logits.float(), labels_crf, mask=mask, reduction="mean"
                    )
                loss = loss + crf_weight * crf_nll
            loss.backward()
            ...
            if crf_weight > 0:
                with torch.amp.autocast("cuda", enabled=False):
                    labels_crf_p = labels.masked_fill(~mask, 0)
                    crf_nll_p = -model.crf(
                        logits_p.float(), labels_crf_p, mask=mask, reduction="mean"
                    )
                loss_p = loss_p + crf_weight * crf_nll_p
            loss_p.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            ...
            if crf_weight > 0:
                with torch.amp.autocast("cuda", enabled=False):
                    labels_crf_ns = labels.masked_fill(~mask, 0)
                    crf_nll_ns = -model.crf(
                        logits.float(), labels_crf_ns, mask=mask, reduction="mean"
                    )
                loss = loss + crf_weight * crf_nll_ns
            optimizer.zero_grad()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="v37: CRF log-likelihood loss weight (0=disabled)",
    )
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v37 (BiLSTM-CRF)")
