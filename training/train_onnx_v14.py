"""kotonoha ONNX accent model trainer (v14).

v13ベースに以下の改善を加えた学習パイプライン:

チェックポイント多様性の向上による精度改善:
- CosineAnnealingWarmRestarts: T_0=20, T_mult=2 (ep 20, 60でLR再始動)
  → 多様なチェックポイントを生成しTop-K avgの効果を最大化
- R-Drop alpha 0.7 (v13の0.5から微増)
- Top-7 checkpoint averaging (v13のTop-5から拡大)
- 32-seed Best-of-N (v13の24から拡大)
- 学習エポック100、patience 25 (WarmRestarts用に延長)

v13から継承:
- Manifold Mixup (alpha=0.3, prob=0.3)
- Feature noise (std=0.02), morpheme_dropout=0.15
- SAM, Reading dropout (p=0.1)
- アーキテクチャはv8と同一 (hidden=256, 3層BiLSTM, ~4.7M params)
...
Usage:
  ...
  cd training
  uv run python train_onnx_v14.py                    # default: 32 seeds
  uv run python train_onnx_v14.py --seeds 0,1,2      # 3 seeds
  uv run python train_onnx_v14.py --scheduler plateau # ReduceLROnPlateau使用
"""
...
def _train_single(  # noqa: PLR0913
    ...
    *,
    ...
    top_k: int = 7,
    scheduler_type: str = "warm_restarts",
    sam_rho: float = 0.0,
    mixup_alpha: float = 0.3,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v14")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument("--stage2-patience", type=int, default=25)
    ...
    parser.add_argument("--top-k", type=int, default=7, help="Top-K checkpoints to avg")
    parser.add_argument(
        "--scheduler",
        choices=["warm_restarts", "cosine", "plateau"],
        default="warm_restarts",
        help="LR scheduler (warm_restarts, cosine, or plateau)",
    )
    parser.add_argument(
        "--sam-rho",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v14 (v13 + WarmRestarts)")
