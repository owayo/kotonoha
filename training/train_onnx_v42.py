r"""kotonoha ONNX accent model trainer (v42).

v38 (80.69%, 14 dim) をベースにモデル容量を拡大して 82%+ を狙う。

## v41 の失敗からの学び
- JVS 全体の v38 pseudo-label 注入は 78.09% と悪化
- Pseudo-label の品質 (JVS に対する v38 精度~60-70%) が低すぎた

## v42 の変更点
1. hidden_dim: 256 -> 384 (+50% 容量)
2. embed_dim: 64 -> 96
3. パラメータ: ~4.7M -> ~10M
4. 他は v38 と完全一致 (14 dim, teacher=v24 argmax feature)

## 期待効果
- 容量増で generalization 改善 → +1-2%
- 多少 overfit するが mixup/dropout/feature noise で抑制

Usage:
  cd training
  uv run python train_onnx_v42.py --seeds 0
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v42")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument("--hidden-dim", type=int, default=384, help="v42: 256->384")
    parser.add_argument("--embed-dim", type=int, default=96, help="v42: 64->96")
    ...
    parser.add_argument(
        ...
        help="v42: start with 3 seeds for capacity experiment",
    )
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v42_states",
        help="v42: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--teacher-cache",
        type=str,
        default="/tmp/v42_teacher_v24_logits.pt",
        help="v42: cache path for precomputed teacher logits",
    )
    parser.add_argument(
        "--val-split-seed",
        type=int,
        default=0,
        help="v42: default 0 (v31 以降の breakthrough split)",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v42 (wide+deep, hidden=384 embed=96)")
