...
v5アーキテクチャ（Embedding層 + カスタムSelf-Attention + 残差接続 + 2層MLP）を
継承した2段階転移学習パイプライン。
...
Usage:
  ...
  cd training
  uv run python train_onnx_v6.py
"""
...
def _train(  # noqa: PLR0913
    ...
    *,
    ...
    print(f"\n{'=' * 65}")
    print("[3/6] Stage 2: Fine-tuning on JSUT")
    print(f"{'=' * 65}")
    ...
    _, pre_acc = _evaluate(model, val_loader, device)
    ...
    print(
        f"\n  [4/6] Up to {stage2_epochs} epochs "
...
def main() -> None:
    ...
    print("  Strategy: JVS pretrain -> JSUT finetune")
    print("=" * 65)
