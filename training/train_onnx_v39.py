r"""kotonoha ONNX accent model trainer (v39).

v38 (80.69%, 14 dim with v24 teacher argmax) をベースに 85%突破を狙う:

## Key idea: Dual-teacher features (v24 + v38)
- FEATURE_DIM = 15 に拡張
- feature[13] = v24 teacher argmax / 20.0 (既存 v38 feature)
- feature[14] = v38 teacher argmax / 20.0 (v38 を新 teacher として追加)
- student は両 teacher の一致/不一致から誤差補正をさらに学習

## v38 -> v39 の期待
- v24 (72%) を teacher に渡して v38 は 80.69% (+8.36%) 達成
- v38 (80.69%) も teacher に追加することで情報源が強化される
- v38 が誤っている位置でも v24 が当たる場合があり、両者の統合が有効

## 継承 (v38)
- teacher argmax を明示的 feature として渡す
- 14 dim -> 15 dim, cont_proj in_features 9->10
- それ以外は v25/v26 と同じ学習手法

Usage:
  cd training
  uv run python train_onnx_v39.py \
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v39")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v39_states",
        help="v39: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v39 (15-dim + v24 + v38 teacher argmax)")
