"""kotonoha ONNX accent model trainer (v11).

v8ベースに以下の改善を加えた学習パイプライン:

v10実験の知見を活かした穏やかな改善:
- Reading dropout (p=0.1) — 訓練時にreading_idsを確率的にドロップ（正則化）
- morpheme_dropout=0.15 — v8の0.1から微増（データ拡張強化）
- Top-5 checkpoint averaging — Top-3から拡張（v8で+0.32%寄与した手法の強化）
- 16-seed Best-of-N — v10で実証済み
- アーキテクチャはv8と同一 (hidden=256, 3層BiLSTM)

v8から継承:
- SAM, R-Drop, ReduceLROnPlateau, morpheme dropout, label smoothing
...
Usage:
  ...
  cd training
  uv run python train_onnx_v11.py                    # default: 16 seeds
  uv run python train_onnx_v11.py --seeds 0,1,2      # 3 seeds
"""
...
class AccentModel(nn.Module):
    ...
    v11: reading_dropout で訓練時にreading_idsを確率的にドロップ（正則化）。
    アーキテクチャはv8と同一 (hidden=256, 3層LSTM)。
    """
    ...
    def forward(
        ...
        """
        # v11: Reading dropout — force model to learn without kana_emb
        if (
            self.training
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v11")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v11")
