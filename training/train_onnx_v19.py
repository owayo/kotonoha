"""kotonoha ONNX accent model trainer (v19).

v13ベースにアクセント句リコンビネーションデータ(2000発話)を追加:

データ拡張:
- JSUTからアクセント句を抽出し、異なる発話の句を組み合わせて新発話を生成
- generate_recombined_data.py で生成した recombined_corpus.json を使用
- v18で mean +0.45%, best +0.21% (71.59%→71.80%) の改善を確認
...
Usage:
  ...
  cd training
  uv run python train_onnx_v19.py                    # default: 48 seeds
  uv run python train_onnx_v19.py --seeds 0,1,2      # 3 seeds
"""
        ...
        if self.augment:
            ...
            for feat, rids in zip(
                ...
                if (
                    ...
                    # v19: Add Gaussian noise to continuous features (indices 5-10)
...
class AccentModel(nn.Module):
    ...
    v19: v11のアーキテクチャ (hidden=256, 3層LSTM) を維持し、
    ...
    def forward(
        ...
        *,
        ...
        # v19: Return hidden for Manifold Mixup before attention + classifier
        if return_hidden:
            return h, h
    ...
    for batch in dataloader:
        ...
        if sam_rho > 0:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v19: Manifold Mixup (in SAM perturbed pass)
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                # v19: Manifold Mixup (probabilistic)
                if mixup_alpha > 0 and random.random() < mixup_prob:
                    mixup_loss = _manifold_mixup_batch(
                        model,
                        features,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v19")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v19 (v13 + Recomb)")
