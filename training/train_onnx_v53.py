...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train kotonoha accent model v53")
    parser.add_argument(
        "--pretrain-data",
        type=Path,
    ...
    parser.add_argument(
        "--state-dir",
        type=str,
        default="/tmp/v53_states",
        help="v53: per-seed state save directory",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="v53: KD with ensemble logits (alpha=0.5 default)",
    )
    parser.add_argument(
        "--kd-temperature",
        type=float,
    ...
    print("=" * 65)
    print("kotonoha accent model trainer v53 (14-dim + ENSEMBLE teacher argmax)")
    ...
    # v52: load ENSEMBLE teacher argmax cache (v17+v20+v24 softmax avg argmax)
    teacher_train_preds: list[np.ndarray] | None = None
    teacher_val_preds: list[np.ndarray] | None = None
    ens_jsut_path = Path("/tmp/v52_ens_teacher_jsut.pt")
    ens_corpus_path = Path("/tmp/v52_ens_teacher_corpus.pt")
    ens_filtered_path = Path("/tmp/v52_ens_teacher_filtered_jvs.pt")
    if ens_jsut_path.exists():
        ens_jsut = torch.load(ens_jsut_path, weights_only=False)
        print(f"\n[v52] loaded ensemble teacher JSUT: {len(ens_jsut)} utts")

        random.seed(args.val_split_seed)
        indices = list(range(len(jsut_utterances)))
        random.shuffle(indices)
        val_size = int(len(indices) * 0.1)
        val_idx_v52 = set(indices[:val_size])
        teacher_train_preds = [
            ens_jsut[i] for i, _ in enumerate(jsut_utterances) if i not in val_idx_v52
        ]
        teacher_val_preds = [
            ens_jsut[i] for i, _ in enumerate(jsut_utterances) if i in val_idx_v52
        ]

        # corpus order: combined_corpus.json + filtered_jvs_v24_t75.json
        if ens_corpus_path.exists():
            ens_corpus = torch.load(ens_corpus_path, weights_only=False)
            teacher_train_preds = teacher_train_preds + list(ens_corpus)
            print(f"[v52] added ensemble teacher corpus: +{len(ens_corpus)}")
        if ens_filtered_path.exists():
            ens_filt = torch.load(ens_filtered_path, weights_only=False)
            teacher_train_preds = teacher_train_preds + list(ens_filt)
            print(f"[v52] added ensemble teacher filtered_jvs: +{len(ens_filt)}")
        print(
            f"[v52] total train teacher preds: {len(teacher_train_preds)}, "
            f"val: {len(teacher_val_preds)}"
        )

    # v53: load ENSEMBLE teacher LOGITS for KD (v17+v20+v24 softmax avg, log)
    ens_log_jsut_path = Path("/tmp/v53_ens_logits_jsut.pt")
    ens_log_corpus_path = Path("/tmp/v53_ens_logits_corpus.pt")
    ens_log_filt_path = Path("/tmp/v53_ens_logits_filtered_jvs.pt")
    if args.kd_alpha > 0 and ens_log_jsut_path.exists():
        ens_log_jsut = torch.load(ens_log_jsut_path, weights_only=False)
        print(f"\n[v53] loaded ensemble logits JSUT: {len(ens_log_jsut)} utts")

        random.seed(args.val_split_seed)
        indices = list(range(len(jsut_utterances)))
        random.shuffle(indices)
        val_size = int(len(indices) * 0.1)
        val_idx_v53 = set(indices[:val_size])
        teacher_train_logits = [
            ens_log_jsut[i]
            for i, _ in enumerate(jsut_utterances)
            if i not in val_idx_v53
        ]

        if ens_log_corpus_path.exists():
            ens_log_corpus = torch.load(ens_log_corpus_path, weights_only=False)
            teacher_train_logits = teacher_train_logits + list(ens_log_corpus)
        if ens_log_filt_path.exists():
            ens_log_filt = torch.load(ens_log_filt_path, weights_only=False)
            teacher_train_logits = teacher_train_logits + list(ens_log_filt)
        print(f"[v53] total train teacher logits: {len(teacher_train_logits)}")
